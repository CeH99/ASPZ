...
Було запущено Docker-контейнер на базі Ubuntu та виконано експерименти зі зміною лімітів відкритих файлів.

У Linux існує два типи лімітів:
- **Soft limit** — поточний (м'який) ліміт, який може вільно змінювати користувач у межах жорсткого ліміту.
- **Hard limit** — максимальний (жорсткий) ліміт, який встановлює верхню межу для м'якого.

У звичайній ОС Linux користувач **root** має привілеї для підвищення жорсткого ліміту. Проте експеримент яскраво демонструє особливості безпеки Docker: у непривілейованому контейнері (запущеному без прапорця `--privileged`) навіть користувач `root` позбавлений частини системних прав (зокрема `CAP_SYS_RESOURCE`). 

Через це, коли `root` зменшує жорсткий ліміт (у нашому випадку до 3000, а потім до 2000), він втрачає можливість підняти його назад і цілком очікувано отримує помилку `Operation not permitted`. Звичайний користувач (student) поводиться аналогічно: він може лише зменшувати ліміти, але не підвищувати їх понад Hard limit.
